...
export class GameScreen implements BaseScreen {
  ...
  private updateTimerDOM(): void {
    ...
    fill.style.animation = grace ? 'timerPulse 0.3s ease-in-out infinite alternate' : 'none'
  }

  // ── Button helpers ────────────────────────────────────────────────────

  private setButtonsEnabled(enabled: boolean): void {
    this.container?.querySelectorAll<HTMLButtonElement>('.gs-ans').forEach((b) => {
      b.disabled = !enabled
    })
  }

  private resetButtonStyles(): void {
    this.container?.querySelectorAll<HTMLElement>('.gs-ans').forEach((btn, i) => {
